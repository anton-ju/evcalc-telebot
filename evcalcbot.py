...
class CalcResultsReport:
    ...
    def print_report(self) -> str:
        ...
        report = []
        report.append(f'avg all in eq %: {round(avg_ai_equity, 2)}')
        report.append(f'total won $: {round(self.total_won - total_bi, 2)}')
        report.append(f'ICM EV diff $: {round(self.icm_evdiff, 2)}')
        report.append(f'Chip won: {round(self.chip_won)}')
        report.append(f'Chip won adj: {round(self.chip_won_adj)}')
        report.append(f'Chip EV diff: {round(self.chip_evdiff)}')
        report.append(f'Total BI $: {round(total_bi, 2)}')
        report.append(f'Avg BI $: {round(avg_bi, 2)}')
